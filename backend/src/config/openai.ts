...
export const openaiClient = new OpenAI({
  ...
  timeout: 120000, // Global timeout 120s to prevent hangs
});

export async function extractDataWithAI(pdfContent: string, fileName: string) {
  console.log(`   🤖 [OpenAI] Starting text analysis for ${fileName} (${pdfContent.length} chars)...`);
  ...
  RESPOND ONLY WITH VALID JSON, NO CODE BLOCKS, NO MARKDOWN. Just the raw JSON object:
  {
    ...
    console.log(`   ✅ [OpenAI] Text analysis completed in ${duration}ms`);
...
export async function extractDataFromImage(imageBuffer: Buffer, fileName: string, pageNum?: number, retryCount = 0): Promise<any> {
  ...
  console.log(`   🤖 [OpenAI] Starting image analysis for ${fileName} page ${pageNum || 1} (${imageBuffer.length} bytes, ${mimeType})...`);
  const startTime = Date.now();
              ...
              text: `Image (page ${pageNum || 1}) from file ${fileName}. First, transcribe any visible text into "rawText". Then extract these fields and provide an "evidence" string for each field showing the exact characters you used. If you cannot read a field, set it to null and leave its evidence as an empty string. Respond ONLY with JSON in this shape:
  {
    ...
    }, { timeout: 90000 }); // 90s timeout per request to prevent freezing

    const duration = Date.now() - startTime;
    console.log(`   ✅ [OpenAI] Image analysis completed in ${duration}ms`);

    let content = message.choices[0]?.message?.content;
    
    if (!content) {
      if (retryCount < 1) {
        console.log(`   ⚠️ [OpenAI] Vision API returned empty content. Retrying (${retryCount + 1}/1)...`);
    ...
    const duration = Date.now() - startTime;
    console.error(`   ❌ [OpenAI] Image analysis failed after ${duration}ms: ${(error as Error).message}`);
    throw error;
  }
}
...
export async function detectDivergencies(
  ...
  console.log(`   🤖 [OpenAI] Starting divergency analysis for ${validDocuments.length} documents...`);
  const startTime = Date.now();
  ...
  Respond with this JSON:
  {
    ...
    const duration = Date.now() - startTime;
    console.log(`   ✅ [OpenAI] Divergency analysis completed in ${duration}ms`);
    ...
    const duration = Date.now() - startTime;
    console.error(`   ❌ [OpenAI] Divergency analysis failed after ${duration}ms: ${(error as Error).message}`);
    throw error;
  }
}
